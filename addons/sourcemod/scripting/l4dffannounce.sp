...
void Event_PlayerDeath(Event event, const char[] name, bool dontBroadcast) 
{
	...
	{
		CPrintToChatAll("[{olive}Unk{default}] %t", "KILL", attacker, victim);
	}	
}
		...
		if (DamageCache[attackerc][i] != 0 && attackerc != i)
		{
			if (IsClientInGame(i))
			{
				...
				switch(g_iCvarAnnounceType)
				{
					...
					{
						if (IsClientInGame(attackerc) && !IsFakeClient(attackerc))
							CPrintToChat(attackerc, "[{olive}Unk{default}] %T", "FF_dealt (C)", attackerc, DamageCache[attackerc][i], victim);
						if (IsClientInGame(i) && !IsFakeClient(i))
							CPrintToChat(i, "[{olive}Unk{default}] %T", "FF_receive (C)", i, attacker, DamageCache[attackerc][i]);
					}
					case 2:
					{
						if (IsClientInGame(attackerc) && !IsFakeClient(attackerc))
